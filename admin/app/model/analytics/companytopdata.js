...
const topcompanyrecuiterdata = async (date) => {
  ...
  const result = await lineupModel.aggregate([
    ...
    },
    {
      ...
      $lookup: {
        from: "companies", // The collection name of the Admin model
...
const topselectcompanyrecuiterdata = async (date) => {
  const { start_date, end_date } = date;
  // Convert the provided date string into a Date object (to match the format of `createdAt`)
  const startOfDay = new Date(start_date);
  const endOfDay = new Date(end_date);
...
const topjoinercompanyrecuiterdata = async (date) => {
  const { start_date, end_date } = date;
  // Convert the provided date string into a Date object (to match the format of `createdAt`)
  const startOfDay = new Date(start_date);
  const endOfDay = new Date(end_date);
